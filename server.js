...
app.post("/api/webhook/weroute", express.urlencoded({ extended: true }), async (req, res) => {
  ...
  try {
    await saveOrder(makeOrder({
      ...
      mode:            "weroute",
      status:          "paid",
      performanceId:   meta.pid   || process.env.PERFORMANCE_ID   || "pilot",
      performanceName: meta.pname || process.env.PERFORMANCE_NAME || "공연",
    ...
    try {
      await saveEvent({
        ...
        event:     "pay_success",
        sessionId: meta.sid || body.ord_num || null,
  ...
  if (q.result_cd === "0000" && q.is_cancel !== "1" && q.ord_num) {
    ...
    try {
      await saveEvent({
        id:        crypto.randomUUID(),
        event:     "pay_success",
        sessionId: meta.sid || q.ord_num || null,
